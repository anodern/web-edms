...
Instance间共享*/
    ...
    /* 下面的三个成员是md5Bean计算过程中用到的3个核心数据，在原始的C实现中
    被定义到md5Bean_CTX结构中
    */
    ...
    */
    ...
    /*
    getmd5BeanofStr是类md5Bean最主要的公共方法，入口参数是你想要进行md5Bean变换的字符串
    返回的是变换完的结果，这个结果是从公共成员digestHexStr取得的．
    */
    public String getmd5BeanofStr(String inbuf) {
        md5BeanInit();
    ...
    */
    private void md5BeanUpdate(byte[] inbuf, int inputLen) {
        ...
        if (inputLen >= partLen) {
            md5BeanMemcpy(buffer, inbuf, index, 0, partLen);
            md5BeanTransform(buffer);
            for (i = partLen; i + 63 < inputLen; i += 64) {
                md5BeanMemcpy(block, inbuf, 0, i, 64);
                md5BeanTransform (block);
            }
            index = 0;
        } else
            i = 0;
    ...
    */
    private void md5BeanFinal () {
        ...
        md5BeanUpdate (PADDING, padLen);
        ///* Append length (before padding) */
        md5BeanUpdate(bits, 8);
